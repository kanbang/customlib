...
AbdlgUdpApp::AbdlgUdpApp(QWidget *parent) :
    QDialog(parent)
{
    udpSocket.bind(5824);
...
/**
 * @brief
    QUdpSocket::bind() 创建用于接受进入的数据报，这个和TCP服务器的QTcpSocket::listen()十分相似。
    无论什么时候当一个或者多个数据报到达时，QUdpSocket就会发送readyRead信号。
    调用QUdpSocket::readDatagram()来读取数据报。
 */
void AbdlgUdpApp::processPendingDatagrams()
{
    QByteArray datagram;
