#include "AdImageWindowApp.h"
#include <ETAllHeader.h>
#include <ui_AddlgImageResize.h>

AdImageWindowApp::AdImageWindowApp(QWidget *parent) :
    QMainWindow(parent)
{
...
void AdImageWindowApp::closeEvent(QCloseEvent *event)
{
    if (okToContinue()) {
        event->accept();
    } else {
...
void AdImageWindowApp::flipHorizontally()
{
    addTransaction(new FlipTransaction(Qt::Horizontal));
}

void AdImageWindowApp::flipVertically()
{
    addTransaction(new FlipTransaction(Qt::Vertical));
}

void AdImageWindowApp::resizeImage()
{
    QDialog dialog;
    Ui::dlgImageResize ui;
    ui.setupUi(&dialog);
...
void AdImageWindowApp::convertTo32Bit()
{
    addTransaction(new ConvertDepthTransaction(32));
}

void AdImageWindowApp::convertTo8Bit()
{
    addTransaction(new ConvertDepthTransaction(8));
}

void AdImageWindowApp::convertTo1Bit()
{
    addTransaction(new ConvertDepthTransaction(1));
}

void AdImageWindowApp::addTransaction(Transaction *transact)
{
...
void AdImageWindowApp::allTransactionsDone()
{
...
void AdImageWindowApp::createActions()
{
...
void AdImageWindowApp::createMenus()
{
...
bool AdImageWindowApp::okToContinue()
{
    if (isWindowModified()) {
        int r = QMessageBox::warning(this, tr("Image Pro"),
...
void AdImageWindowApp::loadFile(const QString &fileName)
{
    QImage newImage;
...
void AdImageWindowApp::saveFile(const QString &fileName)
{
...
void AdImageWindowApp::setCurrentFile(const QString &fileName)
{
    curFile = fileName;
...
QString AdImageWindowApp::strippedName(const QString &fullFileName)
{
